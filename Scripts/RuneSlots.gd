...
@export var initial_unlocks: int = 2
...
func _on_runes_told(runes: Array[RuneWord]) -> void:
	if runes.size() > current_unlocks: 
		error_panel.show_error("You can only use 2 runes!")
		runes = runes.slice(0, current_unlocks)
	if runes[0] != summon_rune:
		error_panel.show_error("First rune must be uus")
		return
	if runes[-1].placement != Enums.RuneSlot.LAST:
			error_panel.show_error("invalid_last_rune")
			return
	else:
